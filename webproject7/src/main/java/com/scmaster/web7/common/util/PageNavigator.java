package com.scmaster.web7.common.util;
...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//페이지당 보이는 게시글 수(ex:5개)
	private int pagePerGroup;		//그룹당 페이지 수(밑에 ◀ 1,2,3,4,5 ▶ 보이는 개수) 
	private int currentPage;		//현재 페이지 (최근 글이 1부터 시작)
	private int totalRecordsCount;	//전체 글 수(103개)
	private int totalPageCount;		//전체 페이지 수(103/5 + 나머지)
	private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작)
	private int startPageGroup;		//현재 그룹의 첫 페이지(5)
	private int endPageGroup;		//현재 그룹의 마지막 페이지(10)
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		//페이지당 글 수, 그룹당 페이지 수, 현재 페이지(화면), 전체 글 수를 전달받음(데이터베이스)
